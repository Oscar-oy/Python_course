...
def isABinarySearchTree(tree):
    
    while tree.getLeftChild():
        if tree.getLeftChild() < tree.getRootVal():
            isABinarySearchTree(tree.getLeftChild())
        else:
            return False

    while tree.getRightChild():
        if tree.getRightChild() > tree.getRootVal():
            isABinarySearchTree(tree.getRightChild())
...
# We just use inorder because it reads before min values in a binary search tree
